...
bst_t *is_idr_success(bst_t *root);
bst_t *delete_func_best(bst_t *root, bst_t *node);
bst_t *bst_remove_recursive(bst_t *root, bst_t *node, int value);
bst_t *bst_remove(bst_t *root, int value);

/**
 * is_idr_success - Returns the minimum value of a binary search tree.
...
/**
 * delete_func_best - Deletes a node from a binary search tree.
 * @root: A pointer to the root node of the BST.
 * @node: A pointer to the node to delete from the BST.
 *
 * Return: A pointer to the new_n root node after deletion.
 */
bst_t *delete_func_best(bst_t *root, bst_t *node)
{
	...
	/* Two children */
	successor = is_idr_success(node->right);
	node->n = successor->n;

	return (delete_func_best(root, successor));
}
...
bst_t *bst_remove_recursive(bst_t *root, bst_t *node, int value)
{
	...
	{
		if (node->n == value)
			return (delete_func_best(root, node));
		if (node->n > value)
