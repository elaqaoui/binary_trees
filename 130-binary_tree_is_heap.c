 ...
 * @i: node index
 * @cdnodes: number of nodes
 * Return: 1 if tree is complete, 0 otherwise
 */
int tree_is_complete(const binary_tree_t *tree, int i, int cdnodes)
{
	if (tree == NULL)
		return (1);
	...
	return (tree_is_complete(tree->left, (2 * i) + 1, cdnodes) &&
		tree_is_complete(tree->right, (2 * i) + 2, cdnodes));
}
...
int binary_tree_is_complete(const binary_tree_t *tree)
{
	...
	cdnodes = binary_tree_size(tree);

	return (tree_is_complete(tree, 0, cdnodes));
}
